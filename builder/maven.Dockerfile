...
ONBUILD RUN sh -c "${UNMAZEDBOOT_BUILDER_BINARY_DEPENDENCIES:-echo installing-no-builder-dependencies}"

# The command used to build the maven app
ONBUILD ARG UNMAZEDBOOT_BUILDER_MAVEN_BUILD_CMD
...
ONBUILD ENV UNMAZEDBOOT_BUILDER_GIT_SHA=${UNMAZEDBOOT_BUILDER_GIT_SHA:-0000000}

# The build BRANCH
ONBUILD ARG UNMAZEDBOOT_BUILDER_GIT_BRANCH
ONBUILD ENV UNMAZEDBOOT_BUILDER_GIT_BRANCH=${UNMAZEDBOOT_BUILDER_GIT_BRANCH:-master}

# The build Pipeline url
ONBUILD ARG UNMAZEDBOOT_BUILDER_GIT_PIPELINE_URL
ONBUILD ENV UNMAZEDBOOT_BUILDER_GIT_PIPELINE_URL=${UNMAZEDBOOT_BUILDER_GIT_PIPELINE_URL:-https://github.com/marcellodesales/unmazedboot-url}

WORKDIR /app/

# Copy the needed files to build a maven project
ONBUILD COPY ./pom.xml pom.xml
ONBUILD COPY ./settings.xml settings.xml
ONBUILD COPY ./src src/

ONBUILD RUN echo "Executing UNMAZEDBOOT_BUILDER_MAVEN_BUILD_CMD=${UNMAZEDBOOT_BUILDER_MAVEN_BUILD_CMD}"
ONBUILD RUN ${UNMAZEDBOOT_BUILDER_MAVEN_BUILD_CMD}
